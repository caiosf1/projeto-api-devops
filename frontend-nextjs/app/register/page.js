...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!success) {
        setError('Falha no registro. Tente outro email.');
        toast.error("Falha no registro. Tente outro email.");
      } else {
      ...
      >
        <Card className="glass border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl shadow-purple-500/10">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-gray-300">Junte-se a nós e organize suas tarefas</CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
              <motion.div 
